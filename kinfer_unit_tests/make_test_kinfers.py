...
def make_step_recipe(
    ...
    def step_fn(
        ...
        random_vals = jnp.array([
            jnp.sin(pseudo_random * 1000.0 + i * 7.0) for i in range(num_joints)
        ])
...
def make_step_echo_recipe(
    ...
    def step_fn(
        ...
        random_vals = jnp.array([
            jnp.sin(pseudo_random * 1000.0 + i * 7.0) for i in range(num_left_joints)
        ])
        random_directions = jnp.sign(random_vals)
...
JOINT_MOTION_LIMITS: dict[str, tuple[float, float]] = {
    ...
    "dof_right_shoulder_roll_03": (math.radians((-95 - (-10)) * JOINT_SAFETY_MARGIN), math.radians((20 - (-10)) * JOINT_SAFETY_MARGIN)),
    "dof_right_shoulder_yaw_02": (math.radians(-95 * JOINT_SAFETY_MARGIN), math.radians(95 * JOINT_SAFETY_MARGIN)),
    "dof_right_elbow_02": (math.radians((0 - 90) * JOINT_SAFETY_MARGIN), math.radians((142 - 90) * JOINT_SAFETY_MARGIN)),
    ...
    "dof_left_shoulder_roll_03": (math.radians((-20 - 10) * JOINT_SAFETY_MARGIN), math.radians((95 - 10) * JOINT_SAFETY_MARGIN)),
    "dof_left_shoulder_yaw_02": (math.radians(-95 * JOINT_SAFETY_MARGIN), math.radians(95 * JOINT_SAFETY_MARGIN)),
    "dof_left_elbow_02": (math.radians((-142 - (-90)) * JOINT_SAFETY_MARGIN), math.radians((0 - (-90)) * JOINT_SAFETY_MARGIN)),
    "dof_left_wrist_00": (math.radians(-100 * JOINT_SAFETY_MARGIN), math.radians(100 * JOINT_SAFETY_MARGIN)),
    
    # Right Leg
    "dof_right_hip_pitch_04": (math.radians((-127 - (-20)) * JOINT_SAFETY_MARGIN), math.radians((60 - (-20)) * JOINT_SAFETY_MARGIN)),
    "dof_right_hip_roll_03": (math.radians((-130 - 0) * JOINT_SAFETY_MARGIN), math.radians((12 - 0) * JOINT_SAFETY_MARGIN)),
    "dof_right_hip_yaw_03": (math.radians(-90 * JOINT_SAFETY_MARGIN), math.radians(90 * JOINT_SAFETY_MARGIN)),
    "dof_right_knee_04": (math.radians((-155 - (-50)) * JOINT_SAFETY_MARGIN), math.radians((0 - (-50)) * JOINT_SAFETY_MARGIN)),
    "dof_right_ankle_02": (math.radians((-13 - 30) * JOINT_SAFETY_MARGIN), math.radians((72 - 30) * JOINT_SAFETY_MARGIN)),
    
    # Left Leg
    "dof_left_hip_pitch_04": (math.radians((-60 - 20) * JOINT_SAFETY_MARGIN), math.radians((127 - 20) * JOINT_SAFETY_MARGIN)),
    "dof_left_hip_roll_03": (math.radians((-12 - 0) * JOINT_SAFETY_MARGIN), math.radians((130 - 0) * JOINT_SAFETY_MARGIN)),
    ...
    "dof_left_ankle_02": (math.radians((-72 - (-30)) * JOINT_SAFETY_MARGIN), math.radians((13 - (-30)) * JOINT_SAFETY_MARGIN)),
}
...
def make_both_arms_sequential_recipe(joint_names: list[str], dt: float, motion_duration: float = 2.0) -> Recipe:
    ...
    def step_fn(
        ...
        left_targets = _make_arm_sequential_targets(
            joint_names, left_arm_joint_names, left_target_indices,
            left_min_offsets, left_max_offsets, bias_vec,
            arm_phase, progress, is_right_arm=False
        )
        
        # Generate targets for right arm when active
        right_targets = _make_arm_sequential_targets(
            joint_names, right_arm_joint_names, right_target_indices,
            right_min_offsets, right_max_offsets, bias_vec,
            arm_phase, progress, is_right_arm=True
        )
...
def make_both_arms_simultaneous_recipe(joint_names: list[str], dt: float, motion_duration: float = 2.0) -> Recipe:
    ...
    def step_fn(
        ...
        left_targets = _make_arm_sequential_targets(
            joint_names, left_arm_joint_names, left_target_indices,
            left_min_offsets, left_max_offsets, bias_vec,
            current_phase, progress, is_right_arm=False
        )
        
        right_targets = _make_arm_sequential_targets(
            joint_names, right_arm_joint_names, right_target_indices,
            right_min_offsets, right_max_offsets, bias_vec,
            current_phase, progress, is_right_arm=True
        )
...
def make_left_arm_sequential_recipe(joint_names: list[str], dt: float, motion_duration: float = 2.0) -> Recipe:
    ...
    def step_fn(
        ...
        targets = _make_arm_sequential_targets(
            joint_names, left_arm_joint_names, target_indices,
            min_offsets, max_offsets, bias_vec,
            current_phase, progress, is_right_arm=False
        )
...
def make_right_arm_sequential_recipe(joint_names: list[str], dt: float, motion_duration: float = 2.0) -> Recipe:
    ...
    def step_fn(
        ...
        # Use helper function to generate targets
        targets = _make_arm_sequential_targets(
            joint_names, right_arm_joint_names, target_indices,
            min_offsets, max_offsets, bias_vec,
            current_phase, progress, is_right_arm=True
        )
...
def make_left_leg_sequential_recipe(joint_names: list[str], dt: float, motion_duration: float = 2.0) -> Recipe:
    ...
    def step_fn(
        ...
        targets = _make_leg_sequential_targets(
            joint_names, left_leg_joint_names, left_arm_joint_names,
            leg_target_indices, arm_target_indices,
            leg_min_offsets, leg_max_offsets, arm_min_offsets, arm_max_offsets,
            bias_vec, current_phase, progress, is_right_leg=False
        )
...
def make_right_leg_sequential_recipe(joint_names: list[str], dt: float, motion_duration: float = 2.0) -> Recipe:
    ...
    def step_fn(
        ...
        # Use helper function to generate targets
        targets = _make_leg_sequential_targets(
            joint_names, right_leg_joint_names, right_arm_joint_names,
            leg_target_indices, arm_target_indices,
            leg_min_offsets, leg_max_offsets, arm_min_offsets, arm_max_offsets,
            bias_vec, current_phase, progress, is_right_leg=True
        )
...
def make_both_legs_sequential_recipe(joint_names: list[str], dt: float, motion_duration: float = 2.0) -> Recipe:
    ...
    def step_fn(
        ...
        left_targets = _make_leg_sequential_targets(
            joint_names, left_leg_joint_names, left_arm_joint_names,
            left_leg_indices, left_arm_indices,
            left_leg_min_offsets, left_leg_max_offsets, left_arm_min_offsets, left_arm_max_offsets,
            bias_vec, leg_phase, progress, is_right_leg=False
        )
        
        # Generate targets for right leg when active
        right_targets = _make_leg_sequential_targets(
            joint_names, right_leg_joint_names, right_arm_joint_names,
            right_leg_indices, right_arm_indices,
            right_leg_min_offsets, right_leg_max_offsets, right_arm_min_offsets, right_arm_max_offsets,
            bias_vec, leg_phase, progress, is_right_leg=True
        )
